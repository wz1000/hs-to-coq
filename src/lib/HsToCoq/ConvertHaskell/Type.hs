...
convertLHsTyVarBndrs ex tvs = for (map unLoc tvs) $ \case
  UserTyVar _ tv   -> Inferred ex . Ident <$> var TypeNS (unLoc tv)
  KindedTyVar _ tv k -> Typed Ungeneralizable ex <$> (pure . Ident <$> var TypeNS (unLoc tv)) <*> convertLType k
...
{-
convertType (HsForAllTy _ tvs ty) = do
...
convertType (HsOpTy _ ty1 op ty2) =
...
convertLHsSigType utvm (HsIB (HsIBRn hs_itvs _) hs_lty ) = do
